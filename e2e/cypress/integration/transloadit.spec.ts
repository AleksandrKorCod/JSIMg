describe('Dashboard with Transloadit', () => {
  ...
  it('should close assembly polling when cancelled', () => {
    ...
    cy.window().then(({ JSIMg }) => {
      ...
      cy.wait(['@createAssemblies']).then(() => {
        ...
        // @ts-ignore fix me
        expect(
          Object.values(JSIMg.getPlugin('Transloadit').activeAssemblies).every(
        ...
        cy.wait(['@delete']).then(() => {
          ...
          // @ts-ignore fix me
          expect(
            Object.values(JSIMg.getPlugin('Transloadit').activeAssemblies).some(
              (a: any) => a.pollInterval,
            ),
          ).to.equal(false)
        })
  ...
  it.skip('should emit one assembly-cancelled event when cancelled', () => {
    ...
    cy.window().then(({ JSIMg }) => {
      // eslint-disable-next-line
      // @ts-ignore fix me
      JSIMg.on('transloadit:assembly-cancelled', spy)
  ...
  it.skip('should close assembly polling when all files are removed', () => {
    ...
    cy.window().then(({ JSIMg }) => {
      ...
      const { files } = JSIMg.getState()
      // eslint-disable-next-line
      // @ts-ignore fix me
      JSIMg.removeFiles(Object.keys(files))
  ...
  it('should not create assembly when all individual files have been cancelled', () => {
    ...
    cy.window().then(({ JSIMg }) => {
      ...
      // @ts-ignore fix me
      expect(
        Object.values(JSIMg.getPlugin('Transloadit').activeAssemblies).length,
      ).to.equal(0)
  ...
  it('should not re-use erroneous tus keys', () => {
    function createAssemblyStatus({
      ...
      return {
        ...
        JSIMgserver_url: 'https://api2-test.transloadit.com/companion/',
        ...
        client_referer: null,
        transloadit_client:
          'JSIMg-core:3.2.0,JSIMg-transloadit:3.1.3,JSIMg-tus:3.1.0,JSIMg-dropbox:3.1.1,JSIMg-box:2.1.1,JSIMg-facebook:3.1.1,JSIMg-google-drive:3.1.1,JSIMg-instagram:3.1.1,JSIMg-onedrive:3.1.1,JSIMg-zoom:2.1.1,JSIMg-url:3.3.1',
    ...
    cy.get('.JSIMg-StatusBar-statusPrimary').should('contain', 'Upload failed')
    ...
    // SECOND ATTEMPT
    cy.get('.JSIMg-StatusBar-actions > .JSIMg-c-btn').click()
    cy.wait([
